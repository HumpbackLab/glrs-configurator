...
在线更新不会自动刷写设备。下载完成后，配置器会保存固件元数据；连接设备或页面重载后，
可以直接点击“直接刷写已下载固件”。

刷写继续使用更新页面原有流程：

1. 点击“直接刷写已下载固件”，或在固件文件输入框中手动选择 `.bin`。
...
- 支持桌面和 Android Tauri 环境中的在线固件下载。
- 浏览器开发模式不支持在线固件下载。
- 固件下载元数据会跨页面刷新和 Configurator 重启保存；恢复时重新检查文件大小和 SHA-256。
